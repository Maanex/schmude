...
public abstract class CustomBlock implements CustomElement {
  ...
  protected void onPlaced(Block b, Player by) { }

  protected void onBroken(Block b, Player by) { }
  ...
  public void placeAt(Block b, Player by) {
    ...
    note.setNote(new Note(getIdElement()));
    b.setBlockData(note);
    onPlaced(b, by);
  }
  ...
  private Instrument getInstrument() {
    ...
    if (group < 0 || group > 3) group = 3;
    return CustomContent.CUSTOMBLOCK_INSTRUMENTS.get(group);
  }
